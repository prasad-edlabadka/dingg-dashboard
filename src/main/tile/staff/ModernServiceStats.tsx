...
export default function ModernServiceStats() {
              ...
              <tbody>
                {Object.keys(displayServices)
                  ...
                  .map((serviceName) => (
                    <>
                      <tr key={serviceName}>
                        <td colSpan={3} className="panel-normal fw-bold text-color">
                          {serviceName}
                        </td>
                      </tr>
                      {displayServices[serviceName].map((service: any, index: number) => (
                        <tr key={`${serviceName}-${index}`}>
                          <td className="text-color border-color-25">{service.stylist}</td>
                          <td className="text-color border-color-25">{service["service count"]}</td>
                          <td className="text-color border-color-25">{currencyFormatter.format(service.price)}</td>
                        </tr>
                      ))}
                    </>
                  ))}
                <tr key="total">
                  <td className="text-color brand-bg fw-bold border-color-25">Total</td>
                  <td className="text-color brand-bg fw-bold border-color-25">
                    {displayServices &&
                      Object.keys(displayServices).length > 0 &&
                      Object.values(displayServices).reduce((acc, val: any) => {
                        return (
                          acc +
                          val.reduce((acc2: number, val2: any) => acc2 + Number.parseInt(val2["service count"]), 0)
                        );
                      }, 0)}
                  </td>
                  <td className="text-color brand-bg fw-bold">
