...
export default function ModernStaff() {
  ...
  const [employees, setEmployees] = useState<ModernEmployeeData[]>([]);
  ...
  useEffect(() => {
    ...
    fetchEmployeeData(callAPIPromise, startDate, endDate).then((data) => {
      ...
      //move item with name 'Store' as the first item in data array
      const storeIndex = data.findIndex((item) => item.name === "Store");
      if (storeIndex !== -1) {
        const [storeItem] = data.splice(storeIndex, 1);
        data.unshift(storeItem);
      }
      setEmployees(data);
      setLoading(false);
    });
  ...
  return (
    <div className="kpi-card-container">
      <DiwaCard varient="primary" loadingTracker={loading} className="glass-panel customer-card">
        <div className="position-relative text-color mb-3">
          <h2 className="panel-title text-color mb-0" style={{ flex: 1 }}>
            Staff Sales Target<p className="panel-sub small mb-0 text-color-50">For services only</p>
          </h2>
