...
function DiwaNavBar({ navs }: { navs: { name: string, icon: any, link: string, iconProps?: object, onClick?: any }[] }) {
  ...
  return (
    <>
      {token !== null ?
        <Navbar bg={darkMode ? "secondary" : "dark"} variant={darkMode ? "light" : "dark"} expand="lg" collapseOnSelect fixed="bottom" data-testid="navbar-parent" className="translucent-navbar">
          <Container>
            {navs.map((nav) => {
              return (
                <Nav.Link
